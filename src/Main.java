...
class EnergyCalculate extends Main
{
    ...
    static void readFileForEnergy() throws IOException
    {
        ...
        do {
            ...
            fname = "C:\\Users\\Joyti\\Project1\\src\\" + fname;
...
//                int start_index=isSafe(path,ind,no_of_slot_needed,temp_slot);
...
//            int start_ind=isSafe(path,1,slot,temp_slot);
        ...
        if(requested_slot!=0)
        {
            //indicate upto slot ind needed
            int slot=requested_slot+2;
//            int start_ind=isSafe(path,0,slot,temp_slot);
...
//            int start_index = isSafe(path, selected_core, no_of_slot_needed,temp_slot);
...
//                int start_index = isSafe(path, i, no_of_slot_needed,temp_slot);
...
//        int temp_slot[][] = new int[number_of_cores][total_slots];
    ...
    // Function to calculate the Fragmentation Measure Metric (FMM)
    public static double calculateFMM(int[] arr, int LFFS, int LBFS) {
        // Calculate the number of gaps, smallest gap, biggest gap, and their counts
        int gaps = 0;
        int smallestGapSize = Integer.MAX_VALUE, biggestGapSize = 0;
        int smallestGapCount = 0, biggestGapCount = 0;

        // Traverse the array to find gaps
        int count = 0;
        for (int i = 0; i < arr.length; i++) {
            if (arr[i] == 0) {
                ...
                if (count > 0) {
                    gaps++; // New gap found
                    if (count < smallestGapSize) {
                        smallestGapSize = count;
                        smallestGapCount = 1;
                    } else if (count == smallestGapSize) {
                        smallestGapCount++;
                    }
                    if (count > biggestGapSize) {
                        biggestGapSize = count;
                        biggestGapCount = 1;
                    } else if (count == biggestGapSize) {
                        biggestGapCount++;
                    }
                    count = 0; // Reset gap counter
                }
            }
        }
        if (count > 0) {
        ...
        // FMM Formula
        double numerator = Math.abs(biggestGapCount * biggestGapSize - smallestGapCount * smallestGapSize) + 1;
        double denominator = Math.abs(biggestGapSize - smallestGapSize) + 1;
        return ((LFFS / (double) LBFS) * gaps * (numerator / denominator) * 100);
    }


    static int isSafe(int path[], int core_number, int requested_slot, int temp_slot[][]) {
        int bestStartIndex = -1;
        double minFragmentationRatio = Double.MAX_VALUE;  // To track the minimum fragmentation ratio
        int totalSlotsInCore = total_slots;

        // Calculate LFFS and LBFS for the current temp_slot
        int LFFS = -1, LBFS = -1;
        for (int i = temp_slot[core_number].length - 1; i >= 0; i--) {
            if (temp_slot[core_number][i] == 1 && LFFS == -1) {
                LFFS = i + 1;
            }
            if (temp_slot[core_number][i] == 0 && LBFS == -1) {
                LBFS = i + 1;
            }
            if (LFFS != -1 && LBFS != -1) break;
        }

        // Step 1: Search for consecutive zeros in temp_slot for the given core
        ...
        for (int i = 0; i < totalSlotsInCore; i++) {
            if (temp_slot[core_number][i] == 0) {
                ...
                if (zeroCount >= requested_slot) {
                    if (zeroCount == requested_slot) {
                        return startIndex; // Exact match found
                    } else {
                        // Calculate FMM for this gap
                        int[] gapArray = Arrays.copyOfRange(temp_slot[core_number], startIndex, startIndex + zeroCount);
                        double fragmentationRatio = calculateFMM(gapArray, LFFS, LBFS);

                        if (fragmentationRatio < minFragmentationRatio) {
                            minFragmentationRatio = fragmentationRatio;
...
//    It does not compare the framentation ratio
