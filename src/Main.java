    ...
    public static void main(String[] args) throws IOException {
      ...
      //  EnergyCalculate obj=new EnergyCalculate();
       // obj.calculateEnergyForPath(1,3,80,"Cost239");
        ...
        //mapMenu();
        //CrosstalkCalculate.initialzeAdjCore();
       // CrosstalkCalculate.calculateXTMenu();
       CoreAllocation obj=new CoreAllocation();
        obj.intialize_available_groups();
        obj.readFileForCoreAllocation();
...
class CoreAllocation extends Main
{
   ...
   // static int core_slots[][]=new int[7][500];

    //static int core_group_size[]={500,166,125,100,71,55,38};
    static int core_group_size[]=new int[number_of_cores];
    static ArrayList<Integer> available_groups[]=new ArrayList[7];
    CoreAllocation()
    {
        for(int i=0;i<number_of_cores;i++)
        {
            if(i==0)
                core_group_size[i]=total_slots/(core_size[i]);
            else
                core_group_size[i]=total_slots/(core_size[i]+2);
        }
    }

    void intialize_available_groups()
    {
        for(int i=0;i<number_of_cores;i++)
        {
            available_groups[i]=new ArrayList<>();
            for(int j=0;j<core_group_size[i];j++)
                available_groups[i].add(0);
        }
    }
     static void allocateCore(int requested_slot)
    {
        int group_no=0;
        for(int i=number_of_cores-1;i>=2;i--)
        {
            if(requested_slot>=core_size[i])
            {
                ...
                int no_of_group_needed = (requested_slot / core_size[ind]);
                int no_of_group_given = 0;
                for (int j = 1; j <= no_of_group_needed; j++) {
                    if ((group_no=isSafe(ind))!=-1)
                        no_of_group_given++;
                }
                int slot_given = no_of_group_given * core_size[ind];
                if(slot_given!=0)
                    System.out.println("core: "+ind+" Index: "+(((group_no +1)* core_size[ind]+2)-1));
                requested_slot = requested_slot - slot_given;
                if(requested_slot==0)
                    break;
            }
        }
        if(requested_slot!=0)
        {
            ...
            int slot=last_allocated_slot_core0+requested_slot-1;
            if(available_groups[0].get(slot)==0)
            {
                for(int k=last_allocated_slot_core0;k<=slot;k++)
                    available_groups[0].set(k,1);
                System.out.println("core: "+0+" Index: "+(slot));
            }
            else
            {
    ...
    static int isSafe(int core_number)
    {
        int group_no=-1;
       for(int i=0;i<available_groups[core_number].size();i++)
       {
           if(available_groups[core_number].get(i)==0) {
               available_groups[core_number].set(i,1);
               group_no=i;
               return group_no;
           }
       }
